"""
Regulation-Aware Visualizations — Publication Figures for the Paper
====================================================================

Builds on the v0.3 visualization module with new plot types specific
to regulation-aware, laterally-routed path planning:

    plot_airspace_map()         — DEM + colored regulatory zones + flight paths
    plot_path_vs_ceiling()      — Altitude profile with terrain floor AND AGL ceiling
    plot_topology_comparison()  — Side-by-side paths WITH vs WITHOUT constraints
    plot_constraint_budget()    — Stacked bar of energy cost per constraint layer
    plot_mission_soc()          — Multi-leg SOC timeline with leg boundaries
    plot_stall_envelope()       — Feasible flight envelope (V vs altitude)
